...
# TODO: Parameterize JDK for refactoring framework so that the user can choose
#       which JRE eclipse should configure projects with by default. Currently,
#       eclipse does not set the JRE for any project which results in compilation
#       errors and prevents refactorings from being performed. Make sure that the
#       user specified JRE is added to project dependencies as a library for all
#       java projects (should the user/alfine-project configuration be able to
#       override this?). We should add this as a default dependency to created
#       projects! (I thought eclipse did this automatically!)
#
# Verify eclipse project configurations by opening the workbench on the generated workspace!
#
# Also, after JRE is set properly, check that we supply all neccessary arguments to inline-method
# descriptors by comparing with refactoring scripts generated for inline-method refactorings.
...
# Replace these with above expressions to generate all data.
declare -a projects=("jacop-bms")
# declare -a types=("inline-method")
declare -a types=("inline-method")
...
for project in "${projects[@]}"; do
    ...
    # Temporary solution for creating `package.config`
    touch $projectSrcDir/packages.config
    echo "jacop-4.6.0=org.jacop.constraints.netflow" >> $projectSrcDir/packages.config

    # Set up eclipse projects and cache refactoring opportunities.
    ./experiment/$framework\
        -data      $preparedWorkspaceDir\
        --prepare\
        --report   "DUMMY-REPORT-DIR"\
        --src      assets/src\
        --lib      assets/lib\
        --cache    $cacheDirName\
        --out      output\
...
function refactor {
    ...
    ## TODO: Write/Get seeds to/from "seeds-$procID.txt" so that we can replay
    ##       refactorings with deterministic seeds.
    ## It the user starts the script with SEEDFILE set, then we should let
    ## processors process refactorings at indices ($procID * N - 1) for whole
    ## numbers N in [1, totalseeds/nproc] and then also process the tail for
    ## affected processors.
    ...
    reportDir=$ALFINE_HOME/experiment/report-proc-$procID

    mkdir -p $reportDir

    reportFile=$reportDir/refactoring-output.txt
    successTrackerFile=$reportDir/successTrackerFile.txt
    failureTrackerFile=$reportDir/failureTrackerFile.txt

    # Refresh the report file before each framework invocation.
    rm    -f $reportFile
    touch    $reportFile

    if [ ! -f $successTrackerFile ]; then
        # We keep these files between framework invocations
        # to prevent the framework from trying to apply
        # refactorings that have already been proven to fail.
        touch $successTrackerFile
        touch $failureTrackerFile
    fi

    seeds="--seed $seedSeed --shuffle $shuffleSeed --select $selectSeed"
    cmd="../../$framework -data . --cache $cacheDirName --src assets/src --lib assets/lib --out output --report $reportDir $seeds $args"
    ...
    # Save tracker files and 
    cp $reportFile $dataDir
    ...
    ## This file contains only the refactoring descriptor
    ## of the performed refactoring.
    cp $procOutputDir/data-record.txt $dataDir

    # Generate data-point-record.sh for benchmarking.
    dataPointRecord=$dataDir/data-point-record.sh

    touch $dataPointRecord
    echo "DPR_SERIAL_NBR=$n"             >> $dataPointRecord
    echo "DPR_ALFINE_PROJECT=$project"   >> $dataPointRecord
    echo "DPR_REFACTORING_TYPE=$type"    >> $dataPointRecord
    echo "DPR_SEED=$seedSeed"            >> $dataPointRecord
    echo "DPR_SHUFFLE_SEED=$shuffleSeed" >> $dataPointRecord
    echo "DPR_SELECT_SEED=$selectSeed"   >> $dataPointRecord
    echo "DPR_CMD=$cmd"                  >> $dataPointRecord
}
...
do
    ...
    # Consider using a thread pool in java so that we can update the success/failure cache
    # between refactorings. By allowing the user to run only the preparation phase and then
    # exit, she can verify eclipse project setup before running the framework again to start
    # refactoring. This would add additional work in writing java code to generate patches
    # and save result files. 
