...
public class RenameLocalVariableOpportunity extends RenameOpportunity {

	public RenameLocalVariableOpportunity(IJavaElement element, Generator generator) {
	...
	protected JavaRefactoringDescriptor buildDescriptor() {
		...
		descriptor = (RenameJavaElementDescriptor)getDescriptor(IJavaRefactorings.RENAME_LOCAL_VARIABLE);
		descriptor.setJavaElement(getElement());

		String oldName = getElement().getElementName();
		String newName = null;
		
		while ((newName = getGenerator().genFieldName()).equals(oldName));
